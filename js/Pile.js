...
(function(ns) {
  ...
  Pile.prototype.add = function(piece) {
    ...
    for (var i = 0; i < this.h; i++) {
      var row = this.grid.map(function(col) { return col[i]; });
        ...
        if (d > 0) {
          for (var i = 0; i < this.w; i++) {
            if (this.grid[i][j]) {
              ...
              var v = (new ns.Vector(0, d * this.cs)).transform(new ns.Matrix(0, 0, -obj.piece.svgItem.a));
              var anim = new ns.Animation(obj.cell, v.at(0), v.at(1), 0, 0, 0, ns.DROP_SPEED[d - 1], ns.FRAME_RATE);
              ...
              setTimeout(animStart, ns.DROP_DELAY * 1000);
            }
          }
        }
      }
  ...
  Pile.prototype.eraseRow = function(r) {
    for (var i = 0; i < this.w; i++) {
      ...
      setTimeout(animStart, ns.CASCADE_EFFECT_DELTA * 1000 * i);
    }
